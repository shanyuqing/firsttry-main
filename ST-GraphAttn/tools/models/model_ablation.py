...
# 消融实验
# 实验1：移除静态图卷积（验证静态结构信息必要性）
class ST_GraphAttn_ablation_1(nn.Module):
    def __init__(self, model_cfg):
        super(ST_GraphAttn_ablation_1, self).__init__()
        input_dim = model_cfg.INPUT_DIM
        hidden_dim1 = model_cfg.HIDDEN_DIM1
        hidden_dim2 = model_cfg.HIDDEN_DIM2
        hidden_dim3 = model_cfg.HIDDEN_DIM3
    ...
    def forward(self, x, sadj, fadj, edge_index, **kwargs):
        ...
        # 不清楚对不对？？！！
        gcn_dynamic = self.DGCN(x_current, fadj)
        gcn_static = torch.zeros_like(gcn_dynamic)  # [N, hidden_dim2]
        gcn_combined = (gcn_dynamic + gcn_static)/2
...
# 实验2：移除动态图卷积
class ST_GraphAttn_ablation_2(nn.Module):
    def __init__(self, model_cfg):
        super(ST_GraphAttn_ablation_2, self).__init__()
        input_dim = model_cfg.INPUT_DIM
        hidden_dim1 = model_cfg.HIDDEN_DIM1
        hidden_dim2 = model_cfg.HIDDEN_DIM2
        hidden_dim3 = model_cfg.HIDDEN_DIM3
    ...
    def forward(self, x, sadj, fadj, edge_index, **kwargs):
        ...
        # 需要修改
        gcn_static = self.SGCN(x_current, sadj)   # [N, hidden_dim2]
        gcn_dynamic = torch.zeros_like(gcn_static)  # [N, hidden_dim2]
        gcn_combined = (gcn_dynamic + gcn_static)/2
...
# 实验三：移除注意力机制
class ST_GraphAttn_ablation_3(nn.Module):
    def __init__(self, model_cfg):
        super(ST_GraphAttn_ablation_3, self).__init__()
        input_dim = model_cfg.INPUT_DIM
        hidden_dim1 = model_cfg.HIDDEN_DIM1
        hidden_dim2 = model_cfg.HIDDEN_DIM2
        hidden_dim3 = model_cfg.HIDDEN_DIM3
...
# 实验4：替换LSTM为线性层
class ST_GraphAttn_ablation_4(nn.Module):
    def __init__(self, model_cfg):
        super(ST_GraphAttn_ablation_4, self).__init__()
        input_dim = model_cfg.INPUT_DIM
        hidden_dim1 = model_cfg.HIDDEN_DIM1
        hidden_dim2 = model_cfg.HIDDEN_DIM2
        hidden_dim3 = model_cfg.HIDDEN_DIM3
        ...
        # Temporal modeling
        self.temporal_fc = nn.Linear(model_cfg.INPUT_DIM, model_cfg.HIDDEN_DIM2)
        # del self.lstm 
    ...
    def forward(self, x, sadj, fadj, edge_index, **kwargs):
        ...
        # Step 1: 移除时间序列建模
        h_lstm = self.temporal_fc(x)        # [N, T, hidden_dim2]
        h_lstm = h_lstm[:, -1, :]              # [N, hidden_dim2]
...
# 实验5：移除门控融合   
class ST_GraphAttn_ablation_5(nn.Module):
    def __init__(self, model_cfg):
        super(ST_GraphAttn_ablation_5, self).__init__()
        input_dim = model_cfg.INPUT_DIM
        hidden_dim1 = model_cfg.HIDDEN_DIM1
        hidden_dim2 = model_cfg.HIDDEN_DIM2
        hidden_dim3 = model_cfg.HIDDEN_DIM3
        ...
        # Gated Fusion
        self.fusion_fc = nn.Linear(model_cfg.HIDDEN_DIM2 * 2, model_cfg.HIDDEN_DIM2)
        # del self.gate 
    ...
    def forward(self, x, sadj, fadj, edge_index, **kwargs):
        ...
        # Step 4: Gated Fusion
        fused = torch.cat([gcn_feat, h_lstm], dim=1)  # [N, hidden_dim2 * 2]
